...
export async function GET() {
  try {
    ...
    const records = await db
      .collection("admission_questions")
      .find({})
      .sort({ uploadedAt: -1 })
      .toArray()
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { db } = await connectDB()
    await db.collection("admission_questions").insertOne({
      fileId,
      examName,
    ...
    return NextResponse.json({ success: true, message: "প্রশ্নপত্র আপলোড সম্পন্ন হয়েছে ✓" })
  } catch (err) {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await db.collection("admission_questions").deleteOne({ fileId: new ObjectId(id) })
    return NextResponse.json({ success: true, message: "প্রশ্নপত্র মুছে ফেলা হয়েছে।" })
  } catch (err) {
