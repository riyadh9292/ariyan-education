...
export default function AdminRoutineUpload() {
  ...
  const handleUpload = async () => {
    ...
    if (!academicYear) return flash("সাল লিখুন।", false)
    ...
    try {
      ...
      if (data.success) {
        flash(data.message ?? "রুটিন আপলোড সম্পন্ন হয়েছে ✓", true)
        setFile(null)
  ...
  const handleDelete = async (fileId: string) => {
    if (!confirm("এই রুটিনটি স্থায়ীভাবে মুছে ফেলবেন?")) return
    try {
      const res = await fetch("/api/routines", {
        method : "DELETE",
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
          <h2 className="font-semibold text-gray-800">ক্লাস রুটিন আপলোড করুন</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-gray-800">আপলোড করা রুটিন</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              মোট {records.length.toLocaleString("bn-BD")}টি রুটিন
            </p>
          </div>
        ...
        {fetching ? (
          ...
          <div className="text-center py-12 text-gray-400">
            ...
            <p>এখনো কোনো রুটিন আপলোড করা হয়নি।</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {records.map(r => (
                  <tr key={r._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-gray-800">{r.academicYear}</td>
