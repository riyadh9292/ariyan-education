...
export default function AdminAdmissionQuestions() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res  = await fetch("/api/admin/admission", { method: "POST", body: fd })
      const data = await res.json()

      if (data.success) {
        flash(data.message ?? "প্রশ্নপত্র আপলোড সম্পন্ন হয়েছে ✓", true)
        setFile(null)
        setExamName("")
  ...
  const handleDelete = async (fileId: string) => {
    if (!confirm("এই প্রশ্নপত্রটি স্থায়ীভাবে মুছে ফেলবেন?")) return
    try {
      const res  = await fetch("/api/admin/admission", {
        method : "DELETE",
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
          <h2 className="font-semibold text-gray-800">ভর্তি পরীক্ষার প্রশ্নপত্র আপলোড</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-gray-800">আপলোড করা প্রশ্নপত্র</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              মোট {records.length.toLocaleString("bn-BD")}টি প্রশ্নপত্র
            </p>
          </div>
